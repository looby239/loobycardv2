...
import '@/styles/templates/template-19.css';
...
export default function Template19({ card, previewMode = false }: TemplateProps) {
  ...
  const eventDateObj = card.event_date ? new Date(card.event_date) : new Date('2026-05-10');
  ...
  const partyTime = card.ceremony_time || '18:00';
  ...
  const coverImage = card.cover_image_url || '/assets/images/template-19/photo1.jpg';
  ...
  return (
    <div className="t19-wrapper">
      ...
      <div className={`envelope-overlay ${opened ? 'open' : ''}`} id="envelope-overlay">
        ...
        <div className="envelope-card">
          <div className="envelope-seal">
            <div className="seal-icon-img"></div>
          </div>
          <div className="envelope-inner">
            {/* Corner decor images */}
            <img src="/assets/images/template-19/header-bottom-01.png" className="decor top-left" alt="" />
            <img src="/assets/images/template-19/header-bottom-01.png" className="decor bottom-right" alt="" />

            <div className="envelope-content">
              <h1 className="couple-names-cover">
                <span className="name-block">{card.groom_name}</span>
                <span className="and-symbol">&amp;</span>
                <span className="name-block">{card.bride_name}</span>
              </h1>
      ...
      <div className="template-container" id="main-container" style={{ display: opened ? 'block' : 'none' }}>
        ...
        <img src="/assets/images/template-19/header-bottom-01.png" className="bg-decor-top" alt="" />
        <img src="/assets/images/template-19/header-bottom-01.png" className="bg-decor-bottom" alt="" />

        {/* Header / Hero Polaroid Photo Section */}
        <header className="wedding-header">
          <div className="hero-frame-container">
            <div className="hero-frame">
              <img src={coverImage} alt="Ảnh cưới" className="hero-image" />
              <div className="polaroid-text">{card.groom_name} &amp; {card.bride_name}</div>
            </div>
          </div>
              ...
              <>
                ...
                <div className="parents-grid">
                  <div className="family-column">
                    ...
                    <span className="parent-title">Ông bà</span>
                    {card.groom_father_name && <h4 className="parent-name">{card.groom_father_name}</h4>}
                    {card.groom_mother_name && <h4 className="parent-name">{card.groom_mother_name}</h4>}
                    {card.groom_address && <p className="parent-address">{card.groom_address}</p>}
                  </div>
                  <div className="family-column">
                    <span className="parent-label">NHÀ GÁI</span>
                    <span className="parent-title">Ông bà</span>
                    {card.bride_father_name && <h4 className="parent-name">{card.bride_father_name}</h4>}
                    {card.bride_mother_name && <h4 className="parent-name">{card.bride_mother_name}</h4>}
                ...
                <div className="ceremony-announcement">
                  ...
                  <div className="couple-names-vertical">
                    ...
                    <span className="and-symbol-vertical">&amp;</span>
                    <div className="couple-name-box">
        ...
        <section className="countdown-section">
          <div className="section-title-wrapper">
            <h2 className="section-title">ĐẾM NGƯỢC NGÀY CHUNG ĐÔI</h2>
          </div>
          <div className="countdown-timer" id="countdown-timer">
            <div className="countdown-item">
              <span className="number" id="days">{timeLeft.days.toString().padStart(2, '0')}</span>
              <span className="label">Ngày</span>
            </div>
            <div className="countdown-item">
              <span className="number" id="hours">{timeLeft.hours.toString().padStart(2, '0')}</span>
              <span className="label">Giờ</span>
            </div>
            <div className="countdown-item">
              <span className="number" id="minutes">{timeLeft.minutes.toString().padStart(2, '0')}</span>
              <span className="label">Phút</span>
            </div>
            <div className="countdown-item">
              <span className="number" id="seconds">{timeLeft.seconds.toString().padStart(2, '0')}</span>
        ...
        {/* Wedding Timeline/Schedule Section */}
            ...
            <form className="rsvp-form" id="rsvp-form" onSubmit={handleRsvpSubmit}>
              ...
              <div className="form-group">
                <select id="guest-count" value={rsvpCount} onChange={(e) => setRsvpCount(Number(e.target.value))}>
                  <option value="" disabled>Số người tham dự đi cùng</option>
          ...
          <section className="gifts">
            ...
            <div className="gift-registry-container">
              <div className="envelope-wrapper" id="gift-modal-trigger" onClick={() => setGiftModalOpen(true)}>
                ...
                <span className="envelope-caption">Nhấn để gửi mừng</span>
              </div>
            </div>
          </section>
        )}
            ...
            <div className="bank-accounts-grid">
              {card.groom_bank_account && (
                <div className="bank-card">
                  <div className="bank-card-header">Chú Rể - {card.groom_name}</div>
                  <div className="qr-code">
                    {getQR(card.groom_bank_name, card.groom_bank_account, card.groom_bank_holder) && (
                      <img src={getQR(card.groom_bank_name, card.groom_bank_account, card.groom_bank_holder)} alt="Groom QR" loading="lazy" />
                    )}
                  </div>
              ...
              {card.bride_bank_account && (
                <div className="bank-card">
                  <div className="bank-card-header">Cô Dâu - {card.bride_name}</div>
                  <div className="qr-code">
                    {getQR(card.bride_bank_name, card.bride_bank_account, card.bride_bank_holder) && (
                      <img src={getQR(card.bride_bank_name, card.bride_bank_account, card.bride_bank_holder)} alt="Bride QR" loading="lazy" />
                    )}
                  </div>
