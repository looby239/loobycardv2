document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Falling Golden Sparkles/Stars Particles Generator ---
  ...
  if (particlesContainer) {
    ...
    for (let i = 0; i < maxParticles; i++) {
      ...
      particle.innerText = starSymbols[Math.floor(Math.random() * starSymbols.length)];
      
      // Randomize styles
      const leftPos = Math.random() * 100; // percentage
      const fontSize = 12 + Math.random() * 14; // 12px to 26px
      ...
      // Apply gold/bronze color shades
      const goldShades = ['#B97D3E', '#dfb279', '#C5A059', '#e8dcc4', '#b5945b'];
      particle.style.color = goldShades[Math.floor(Math.random() * goldShades.length)];
  ...
  const weddingDate = new Date('August 2, 2026 11:30:00').getTime();
  ...
  if (countdownContainer) {
    ...
    const updateCountdown = () => {
      ...
      if (distance < 0) {
        ...
        countdownContainer.innerHTML = '<div class="countdown-item" style="width: 100%;"><span style="font-size: 1.1rem; color: #B97D3E;">Đã đến ngày chung đôi!</span></div>';
        return;
      }
  ...
  if (rsvpForm) {
    rsvpForm.addEventListener('submit', (e) => {
      ...
      if (guestStatus === 'yes') {
        ...
        responseMsg = `Cảm ơn ${guestName} đã xác nhận tham dự ${countText}! Rất hân hạnh được đón tiếp bạn chung vui cùng gia đình chúng tôi tại Thung Lũng Cá Đà Lạt.`;
      } else {
        responseMsg = `Cảm ơn ${guestName} đã gửi phản hồi. Rất tiếc vì bạn không thể ghé thăm và chung vui cùng gia đình chú rể, cô dâu.`;
      }
  ...
  const photosList = [
    ...
    '../../../assets/images/template-15/photo5.jpg',
    '../../../assets/images/template-15/photo6.jpg'
  ];
  ...
  function showCustomAlert(message, title = 'Thông Báo') {
    ...
    alertBox.style.cssText = `
      ...
      max-width: 380px;
      text-align: center;
      box-shadow: 0 16px 32px rgba(185, 125, 62, 0.12);
